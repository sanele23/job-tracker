...
export function JobForm({ job, defaultStatus = 'wishlist', onClose }: JobFormProps) {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <Field label="Company *" error={errors.company?.message}>
          <input {...register('company')} placeholder="e.g. Stripe" className={inputClass} />
        </Field>
        <Field label="Role *" error={errors.role?.message}>
          <input {...register('role')} placeholder="e.g. Senior Frontend Engineer" className={inputClass} />
        </Field>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4">
        <Field label="Min Salary">
          <input {...register('salary_min')} type="number" placeholder="150000" className={inputClass} />
        </Field>
        <Field label="Max Salary" error={errors.salary_max?.message}>
          <input {...register('salary_max')} type="number" placeholder="200000" className={inputClass} />
        </Field>
      </div>

      {/* date */}
      <Field label="Date Applied">
        <input {...register('date_applied')} type="date" className={inputClass} />
      </Field>

      {/* url */}
      <Field label="Job URL" error={errors.url?.message}>
        <input {...register('url')} placeholder="https://company.com/careers/role" className={inputClass} />
      </Field>

      {/* contact */}
      <div className="grid grid-cols-2 gap-4">
        <Field label="Contact Name">
          <input {...register('contact_name')} placeholder="Sarah Chen" className={inputClass} />
        </Field>
        <Field label="Contact Email" error={errors.contact_email?.message}>
          <input {...register('contact_email')} placeholder="sarah@company.com" className={inputClass} />
        </Field>
      </div>
      ...
      {/* tags */}
      <Field label="Tags">
        <input {...register('tags')} placeholder="react, typescript, remote (comma-separated)" className={inputClass} />
      </Field>
